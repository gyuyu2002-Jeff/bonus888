...
function renderInputTable() {
  ...
  let headerHtml = `
    <th>月份</th>
    <th>個人業績 (萬元)<br><span class="th-sub">目標 / 實際</span></th>
    <th>個人 ${col2Name}<br><span class="th-sub">目標 / 實際</span></th>
    <th>個人 ${col3Name}<br><span class="th-sub">目標 / 實際</span></th>
  `;

  if (role === 'supervisor') {
    headerHtml += `
      <th>小組總業績 (萬元)<br><span class="th-sub">目標 / 實際</span></th>
      <th>小組 ${col2Name}<br><span class="th-sub">目標 / 實際</span></th>
      <th>小組 ${col3Name}<br><span class="th-sub">目標 / 實際</span></th>
    `;
  }
  ...
  for (let m = 1; m <= monthsCount; m++) {
    ...
    let rowHtml = `
      <td>第 ${m} 個月${isFirstMonth ? '<br><small class="text-danger">(首月免計)</small>' : ''}</td>
      <td>
        <input type="number" step="any" id="t-sales-${m}" value="${tSales}" ${readOnlyAttr}> /
        <input type="number" step="any" id="a-sales-${m}" value="" placeholder="實際">
      </td>
      <td>
        <input type="number" id="t-dev-${m}" value="${tNewDev}" ${readOnlyAttr}> /
        <input type="number" id="a-dev-${m}" value="" placeholder="實際">
      </td>
      <td>
        <input type="number" id="t-mach-${m}" value="${tNewMach}" ${readOnlyAttr}> /
        <input type="number" id="a-mach-${m}" value="" placeholder="實際">
      </td>
    `;
    ...
    if (role === 'supervisor') {
      rowHtml += `
        <td>
          <input type="number" step="any" id="tg-sales-${m}" value="${tgSales}" ${readOnlyAttr}> /
          <input type="number" step="any" id="ag-sales-${m}" value="" placeholder="實際">
        </td>
        <td>
          <input type="number" id="tg-dev-${m}" value="${tgNewDev}" ${readOnlyAttr}> /
          <input type="number" id="ag-dev-${m}" value="" placeholder="實際">
        </td>
        <td>
          <input type="number" id="tg-mach-${m}" value="${tgNewMach}" ${readOnlyAttr}> /
          <input type="number" id="ag-mach-${m}" value="" placeholder="實際">
        </td>
      `;
    }
...
function updatePassConditions() {
  ...
  let html = `
    ...
    <ul class="ref-target-list">
      <li><span>累計業績總目標:</span> <strong>${tSales.toFixed(1)} 萬元</strong></li>
      <li><span>累計 ${m2Name} 目標:</span> <strong>${tDev} 台</strong></li>
      <li><span>累計 ${m3Name} 目標:</span> <strong>${tMach} 台</strong></li>
    </ul>
